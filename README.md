...
- **Repo checklist** with status badges after each run (✅ Synced · ⛔ Blocked ·
  ...
  Each repo row has its own **Dry-run** and **Sync** buttons to target just
  that repo. Repos without a GitHub remote show a **Publish to GitHub…**
  button instead of Sync — clicking it opens the Create GitHub repo dialog
  pre-filled for that repo.
- **Stale indicator** — repos not synced in 3+ days (or never synced) show a
  🕒 badge with how long it's been.
...
- **Leak report** — when a repo is blocked, the output pane appends a
  formatted summary listing exactly which file and rule triggered the gate,
  so you know what to fix without opening the log.

**Output pane:** ANSI colour codes from gitleaks are stripped so the output
is always plain readable text.
...
The app lives in the macOS menu bar as a coloured dot (green = all clear,
red = attention needed, grey = no runs yet). Closing the main window does
**not** quit the app — it hides to the tray so scheduled syncs keep running
in the background. To reopen: click the tray dot or the Dock icon. To quit
fully: choose **Quit** from the tray menu.
...
- The packaged app resolves `gitleaks` / `git` / `gh` explicitly (checking
  ...
  by launchd too.
- `build_app.sh` deletes `build/` and `dist/` after installing to
  `/Applications`, so Spotlight never indexes a second copy.
